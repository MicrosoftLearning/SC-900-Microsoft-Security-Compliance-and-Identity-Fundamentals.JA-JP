...
1. Microsoft Sentinel ページに、Log Analytics ワークスペースが一覧表示されます。  これは、Microsoft Sentinel のインスタンスが作成され、そこに Log Analytics ワークスペースが追加されたことを示します。 そうでない場合は、次の手順に従います。
...
1. [アクセス制御] ページで、**[自分のアクセスの表示]** を選択します。  ほとんどの場合、このサブスクリプションに対して既に構成されているカスタム ロールの割り当てが表示され、必要なアクセス許可が付与されます。  このカスタム ロールは、ラボ環境を提供する承認されたラボ ホスト (ALH) によって設定されています。 ただし、デモ目的では、Sentinel 固有のロールを表示することをお勧めします。  ウィンドウの右上隅の **[X]** を選択して、割り当てウィンドウを閉じます。
...
1. 左側のナビゲーション パネルで、**[コンテンツ管理]** を展開して、**[コンテンツ ハブ]** を選択します。
