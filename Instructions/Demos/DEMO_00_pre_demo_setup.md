...
このセットアップは、2 つの個別のタスクで構成されています。  1 番目のタスクは、デモ環境に Azure Pass が含まれている場合にのみ適用され、推奨されます。 2 番目のタスクは、Microsoft 監査ログの有効化に重点を置いており、自分の環境で Azure Pass を使用するかどうかに関係なく、適用され、推奨されます。
