...
1. 左側のナビゲーション ウィンドウで **[Entra ID]** が展開されていることを確認し、**[ユーザー]** を選択してから **[すべてのユーザー]** を選択します。
...
1. 左側のナビゲーション パネルで **[Entra ID]** の下までスクロールして、**[条件付きアクセス]** を選択します。
...
1. [ユーザー] で **[0 人のユーザーとエージェント (プレビュー) が選択されました]** を選択します。
...
1. **[このポリシーが適用される対象を選択する]** の下のフィールドで下矢印を選び、使用できるオプションを確認します。  既定の設定 **Resources (旧称クラウド アプリ)** のままにします。  **[含める]** タブに下線が付いていることを確認します。  **[リソースの選択]** を選択し、**[特定のリソースの選択]** で **[なし]** を選択します。  [クラウド アプリの選択] ウィンドウが開きます。
...
このタスクでは、ユーザー Debra Berger の視点から、条件付きアクセス ポリシーの影響を確認します。 最初に、条件付きアクセス ポリシーに含まれないアプリケーション (**https://login.microsoftonline.com** の Microsoft 365 ポータル) にサインインします。  次に、条件付きアクセス ポリシーに含まれるアプリケーション (**https://portal.azure.com** の Azure portal) でこのプロセスを繰り返します。  このポリシーでは、Azure portal を含むすべての Microsoft 管理ポータルへのアクセスがブロックされることを思い出してください。  

   >**注**: セキュリティ上の理由から、すべてのポータルにアクセスするすべてのユーザー アカウントで MFA を使用する必要があります。  MFA 要件は、このラボの演習では扱いません。
