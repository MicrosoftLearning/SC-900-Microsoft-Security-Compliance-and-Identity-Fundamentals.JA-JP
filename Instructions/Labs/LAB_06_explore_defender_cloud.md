...
lab:
  ...
  description: このラボでは、Microsoft Defender for Cloud について学びます。
  duration: 30 minutes
  level: 200
  islab: true
  primarytopics:
...
このラボでは、Microsoft Defender for Cloud について学びます。 Azure サブスクリプションは Authorized Lab Hoster (ALH) から取得されるため、一部の機能が制限されることや、読み込みに少し時間がかかる場合があります。
...
1. Azure サービスのホーム ページが表示されているはずです。  既にブラウザーを閉じている場合は Microsoft Edge を開きます。 アドレス バーに「**`https://portal.azure.com`**」と入力し、管理者の資格情報でサインインします。
...
1. Microsoft Defender for Cloud の [概要] ページで、そのページで利用可能な情報に注目します (評価されたリソースとアクティブな推奨事項が 0 件の場合は、ブラウザー ページを更新します。これには、数分かかる場合があります)。  ページの上部の情報には、Azure サブスクリプションの数、評価済みのリソースの数、アクティブな推奨事項の数、セキュリティ アラートが含まれます。  ページの本文には、セキュリティ態勢、規制コンプライアンス、分析情報などを表す複数のカードがあります。  

   >**注**: 通常、管理者によって割り当てられる必要がある Microsoft Defender for Cloud の既定のポリシー イニシアチブは、Azure サブスクリプションのセットアップの一環として既に割り当て済みです。 ただし、Azure によって初期スコアが反映されるまでに最大 24 時間の遅延が発生する場合があるため、セキュア スコアは 0% と表示されます。
...
1. メインの左ナビゲーション パネルで、**[クラウド セキュリティ]** を展開して **[規制コンプライアンス]** を選択します。  
   >**注**: コンプライアンスを計算するサブスクリプションが表示されないのは、情報が表示されるまでに最長 24 時間かかることがあるためです。 タスク 2 に進みます。  情報が表示されたら、次の手順に進みます。
