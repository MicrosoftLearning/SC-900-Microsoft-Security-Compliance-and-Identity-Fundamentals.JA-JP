...
1. [クエリ条件] ページでは、作成できるクエリに基づいてアイテムのホールドを作成できます。  クエリ ビルダーを使用してクエリを作成できますし、上級ユーザーの場合は KQL エディターを使用することもできます。 この演習では、指定したユーザーの指定の場所にあるすべてのコンテンツをホールドに保存して、クエリを作成しないようにします。
...
ホールドを設定して、検索クエリを作成します。  検索が完了すると、電子情報開示では、今後の調査のために結果のエクスポートやダウンロードなどのアクションがサポートされます。   注: 電子情報開示 (Standard) ケースに関連する検索は、Microsoft Purview ポータルの [コンテンツ検索] ページの一覧には表示されません。 これらの検索は、関連付けられた電子情報開示 (Standard) ケースの [検索] ページにのみ一覧表示されます。
