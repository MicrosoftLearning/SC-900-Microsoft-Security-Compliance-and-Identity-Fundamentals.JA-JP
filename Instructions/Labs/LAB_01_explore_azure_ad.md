...
7. 次に、新しいブラウザー ウィンドウを開いて、アドレス バーに、「**portal.azure.com**」と入力します。  既に admin@WWLxZZZZZZ.onmicrosoft.com としてサインインしており、元々それらの同じ資格情報を Azure Pass を引き換えるのに使用していたため、Azure portal にアクセスするときには管理者としてログインしているはずです。  ページの右上隅のメールアドレスを確認し、ユーザー アイコンにマウスをホバリングして、これを確認できます。
...
2. 左側のナビゲーション ウィンドウで、**[ユーザー]** を選択します。  ユーザーによりテナントがすでに構成されていることに注意してください。

3. ページの上部で、**[+ 新しいユーザー]** を選択します。

4. **[ユーザーの作成]** が既に選択されているはずです。選択されていない場合は、このオプションを選択します。
...
7. **グループとロール** を構成します。
    ...
    3. [ロール] の横にある **[ユーザー]** を選択します。 ディレクション ロールの一覧が表示されます。  下にスクロールして、さまざまな組み込みのロールを確認します。さまざまなロールを確認しますが、ユーザー ロールを変更しないでください。  ページの右上隅で **[X]** を選択して、このウィンドウを閉じます。
...
9. ページの下部にある **[作成]** ボタンを選択します。

10. ユーザー一覧にユーザーが表示されることを確認します (名前はアルファベット順に一覧表示されます)。

11. ユーザーの一覧から、作成したユーザー **"Sara Perez"** を選択します。  [プロファイル] ページが開きます。

12. 左ナビゲーション パネルにユーザーに構成できるさまざまなオプションが表示されます。  **[グループ]** を選びます。  ここで、グループに関する追加情報を表示できます。  操作グループが一覧表示されることを確認します (グループ割り当てを表示するのに数分かかる場合があります)。  注: ユーザーを作成したときには 1 つのグループのみを割り当てましたが、Contoso グループも表示されます。  これは、事前構成されたポリシーの結果です。テナントでは、新しいユーザーは Contoso グループに自動的に割り当てられます。

13. 左側のナビゲーション ウィンドウで、**[ライセンス]** を選択します。  このユーザーにライセンスが割り当てられていないことに注意してください。  

14. ライセンスを追加するには、メイン ウィンドウの上部で **[+ 割り当て]** を選択します。

15. [ライセンスの選択] で、**[Office 365 E3]** と **[Windows 10 Enterprise E3]** を選択してから、画面の下部にある **[保存]** ボタンを選択します。 画面の右上隅の通知には、ライセンス割り当てが成功したことが表示されるはずです。

16. 画面右上で **[X]** を選択して、[ライセンスの割り当て] ウィンドウを閉じます。

17. ページの上部にある **[更新] アイコン** を選択して、ライセンスの割り当てを確認します。

18. 画面 (階層リンク) 左上の、"Sara Perez | ライセンス" と表示されている場所の上にある **[Contoso]** を選択することで、Azure Active Directory の [Contoso - 概要] ページに戻ります。

19. Azure Active Directory でのユーザーの作成および構成に成功しました。

20. 画面の右上隅のメールアドレスの横にある [ユーザー] アイコンをクリックして、すべてのブラウザー タブからサインアウトします。 次に、すべてのブラウザー ウィンドウを閉じます。
...
8. 画面の右上隅のメール アドレスの横にある [ユーザー] アイコンをクリックして、すべてのブラウザー タブから **サインアウト** します。 次に、すべてのブラウザー ウィンドウを閉じます。
